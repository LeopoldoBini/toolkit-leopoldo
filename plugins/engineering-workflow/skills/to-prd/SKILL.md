...
Exception: if a previous exploration (a prototype, spike, or REPL session) produced a snippet that encodes a decision more precisely than prose can — state machine, reducer, schema, type shape — inline it within the relevant decision and note briefly where it came from. Trim to the decision-rich parts; not a working demo, just the important bits.
